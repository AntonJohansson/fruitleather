...
pub fn err(comptime format: []const u8, args: anytype) void {
    ...
    var tty_config = std.debug.detectTTYConfig(std.io.getStdErr());

    tty_config.setColor(stderr, .Red) catch return;
    nosuspend stderr.writeAll("Error: ") catch return;
    tty_config.setColor(stderr, .Reset) catch return;
